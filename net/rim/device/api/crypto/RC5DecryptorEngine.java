...
public final class RC5DecryptorEngine implements SymmetricKeyDecryptorEngine {
   ...
   public RC5DecryptorEngine(RC5Key key, int blockLength) {
   ...
   public RC5DecryptorEngine(RC5Key key, int blockLength, int numberOfRounds) {
      if (key != null && (blockLength == 8 || blockLength == 16) && numberOfRounds >= 0 && numberOfRounds <= 255) {
         this._cryptoToken = key.getRC5CryptoToken();
         this._context = this._cryptoToken.initializeDecrypt(key.getCryptoTokenData(), blockLength, numberOfRounds);
