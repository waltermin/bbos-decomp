...
final class FocusGroup {
   ...
   final void addLeaf(ModelInteractorImpl modelInteractor, Attributes attributes) {
   ...
   final void resolveIds(ModelInteractorImpl modelInteractor) {
   ...
   final void enableItem(int handle, boolean enable) {
      int count = this._items.length;
      ...
      for (int i = 0; i < count; i++) {
         ...
         if (item instanceof Integer) {
            ...
            if (handle == integer) {
               if (enable) {
                  if (this._disables != null) {
                     this._disables[i] = false;
                     return;
                  }
               } else {
                  if (this._disables == null) {
                     this._disables = new boolean[this._items.length];
                  }

                  this._disables[i] = true;
               }
         ...
         if (item instanceof FocusGroup) {
            FocusGroup focusGroup = (FocusGroup)item;
            focusGroup.enableItem(handle, enable);
         }
      }
   }
}
