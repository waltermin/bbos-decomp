...
final class RouteSyncObject implements LocationSyncable, Persistable {
   ...
   public final boolean load(DataBuffer buffer, int version) {
      try {
         if (ConverterUtilities.findType(buffer, 2)) {
            ...
            Route loc = (Route)conv.getLocation(this);
            this._label = loc._routeName;
   ...
   public final boolean save(DataBuffer data, int version) {
      ConverterUtilities.writeByteArray(data, 2, this._data);
      return true;
   }
   ...
   RouteSyncObject(int uid, String label, byte[] data, SimpleFolder folderHeirarchy) {
      this._label = label;
      this._uid = uid;
      this._data = data;
