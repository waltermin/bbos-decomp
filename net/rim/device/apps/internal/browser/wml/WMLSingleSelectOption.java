...
final class WMLSingleSelectOption {
   ...
   private WMLSingleSelectInputField _select;
   private WMLRadioButtonField _radioButton;

   WMLSingleSelectOption(WMLSingleSelectInputField select, WMLVariable value, WMLVariable title, Verb onPick) {
      this._value = value;
      this._title = title;
      this._onPick = onPick;
      this._select = select;
      this._radioButton = new WMLRadioButtonField("", this.getTitle());
      this._radioButton.setChangeListener(this._select);
   }

   final void setLabel(String label) {
      this._initText = label;
      this._radioButton.setLabel(label);
   }
   ...
   final WMLRadioButtonField getRadioButton() {
   ...
   final void updateValues() {
      if (this._initVarStarts != null && this._initVarStarts.length != 0) {
         ...
         this._radioButton.setLabel(text);
      }
   }
}
