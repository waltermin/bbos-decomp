...
public class DatakeyCryptoSmartCardSession extends CryptoSmartCardSession implements DatakeyPKCS11Constants {
   ...
   @Override
   protected int getRemainingLoginAttemptsImpl() throws SmartCardUnsupportedOperationException {
      throw new SmartCardUnsupportedOperationException();
   }

   static CommandAPDU getSelectFileCommand(byte[] file) {
      return new CommandAPDU((byte)0, (byte)-92, (byte)8, (byte)0, file, 11);
   }

   private DatakeyFileHeader selectFile(byte[] file) throws SmartCardException {
      ...
      if (response.checkStatusWords((byte)-112, (byte)0)) {
         ...
         if (fileHeaderData == null) {
            throw new SmartCardException("No file header data");
         } else {
   ...
   @Override
   protected boolean loginImpl(String password) throws SmartCardLockedException {
      CommandAPDU command = new CommandAPDU((byte)0, (byte)32, (byte)1, (byte)0);
   ...
   protected byte[] getRandomBytesImpl(int maxBytes) {
      ...
      CommandAPDU command = new CommandAPDU((byte)0, (byte)76, (byte)0, (byte)0, maxBytes);
      ResponseAPDU response = new ResponseAPDU();
      this.sendAPDU(command, response);
      return response.checkStatusWords((byte)-112, (byte)0) ? response.getData() : null;
   }

   private DatakeyFile getFile(byte[] file) throws SmartCardAccessDeniedException, SmartCardException {
      DatakeyFileHeader fileHeader = this.selectFile(file);
      if (fileHeader == null) {
         return null;
      }

      CommandAPDU command = new CommandAPDU((byte)0, (byte)-80, (byte)0, (byte)0);
      ResponseAPDU response = new ResponseAPDU();
      int offset = 0;
   ...
   private Vector findMDFiles(int numFiles) {
      CommandAPDU[] commands = new CommandAPDU[numFiles];

      for (int i = 0; i < numFiles; i++) {
         commands[i] = new CommandAPDU((byte)0, (byte)-80, (byte)0, (byte)i, 11);
      }

      CommandAPDUGroup commandAPDUGroup = new CommandAPDUGroup(commands);
      ResponseAPDUGroup responseAPDUGroup = new ResponseAPDUGroup(numFiles);
   ...
   protected CryptoSmartCardKeyStoreData[] getKeyStoreDataArrayImpl() {
      ...
      // 013: new net/rim/device/api/util/MultiMap
      // 016: dup
      ...
      // 0e3: goto 1e5
      // 0e6: aload 2
      // 0e7: new net/rim/device/internal/util/ByteArray
      // 0ea: dup
      // 0eb: aload 14
      ...
      // 10b: goto 1e5
      // 10e: aload 2
      // 10f: new net/rim/device/internal/util/ByteArray
      // 112: dup
      // 113: aload 14
      ...
      // 191: ifeq 1b1
      // 194: aload 18
      // 196: checkcast net/rim/device/api/crypto/RSAPublicKey
      // 199: astore 20
      // 19b: aload 2
      // 19c: new net/rim/device/internal/util/ByteArray
      // 19f: dup
      // 1a0: aload 20
      ...
      // 1bd: ifnull 1dd
      // 1c0: aload 2
      // 1c1: new net/rim/device/internal/util/ByteArray
      // 1c4: dup
      // 1c5: aload 20
      ...
      // 217: checkcast net/rim/device/internal/util/ByteArray
      // 21a: astore 11
      // 21c: aload 2
      // 21d: aload 11
      ...
      // 240: aload 15
      // 242: dup
      // 243: instanceof net/rim/device/api/crypto/certificate/Certificate
      // 246: ifne 24d
      // 249: pop
      // 24a: goto 255
      // 24d: checkcast net/rim/device/api/crypto/certificate/Certificate
      // 250: astore 13
      // 252: goto 22a
      // 255: aload 15
      // 257: dup
      // 258: instanceof net/rim/device/api/crypto/PrivateKey
      // 25b: ifne 262
      // 25e: pop
      // 25f: goto 26a
      // 262: checkcast net/rim/device/api/crypto/PrivateKey
      // 265: astore 14
      // 267: goto 22a
      // 26a: aload 15
      // 26c: dup
      ...
      // 376: astore 25
      // 378: new net/rim/device/api/crypto/DSACryptoSystem
      // 37b: dup
      // 37c: aload 25
      ...
      // 388: astore 26
      // 38a: new net/rim/device/api/crypto/DSAPrivateKey
      // 38d: dup
      // 38e: aload 26
      // 390: aload 20
      ...
      // 3b9: bipush 1
      // 3ba: isub
      // 3bb: new net/rim/device/api/crypto/CryptoSmartCardKeyStoreData
      // 3be: dup
      // 3bf: aconst_null
      // 3c0: aload 13
   ...
   public void sha1Hash() {
      ...
      CommandAPDU command = new CommandAPDU((byte)0, (byte)-128, (byte)5, (byte)0, dataToHash, 20);
      ResponseAPDU response = new ResponseAPDU();
      this.sendAPDU(command, response);
      if (!response.checkStatusWords((byte)-112, (byte)0)) {
         ;
   ...
   private void pkcs1Format(byte[] input, byte[] output, int outputOffset, int outputBlockLength) throws CryptoTokenException {
      if (input != null && output != null && outputOffset >= 0 && output.length - outputBlockLength >= outputOffset) {
         ...
         if (inputLength > outputBlockLength - 11) {
            throw new CryptoTokenException("Message too long");
         }
         ...
         throw new CryptoTokenException("Illegal argument");
      }
   }
   ...
   void signDecryptRSA(
   ...
   ) throws CryptoUnsupportedOperationException, SmartCardException {
      if (cryptoSystem != null && privateKeyData != null && input != null && output != null) {
         ...
         if (input.length >= inputOffset + modulusLength && output.length >= outputOffset + modulusLength) {
            CommandAPDU command = new CommandAPDU((byte)0, (byte)(operation == 1 ? 90 : 84), (byte)(operation == 1 ? 1 : 1), (byte)0, null);
            ...
            if (operation == 1) {
               ...
               if (data == null) {
                  throw new CryptoUnsupportedOperationException("Unsupported data formatting");
               }
            ...
            if (response.checkStatusWords((byte)-112, (byte)0)) {
               ...
               throw new SmartCardException(
                  "Invalid response. SW1: " + Integer.toHexString(response.getSW1() & 255) + " SW2: " + Integer.toHexString(response.getSW2() & 255)
               );
            }
         } else {
   ...
   void signDSA(
      ...
      if (cryptoSystemData != null && privateKeyData != null && digest != null && r != null && s != null) {
         if (r.length >= rOffset + 20 && s.length >= sOffset + 20 && digestLength == 20 && digest.length >= digestOffset + 20) {
            CommandAPDU command = new CommandAPDU((byte)0, (byte)70, (byte)0, (byte)0, 40);
            command.appendLcData(new byte[]{63, 0});
            ...
            if (response.checkStatusWords((byte)-112, (byte)0)) {
               ...
               throw new SmartCardException("Invalid response. SW1: " + response.getSW1() + " SW2: " + response.getSW2());
            }
         } else {
            throw new IllegalArgumentException("invalid parameter length");
         }
      } else {
         throw new IllegalArgumentException("null parameter in signDSA");
      }
   }
   ...
   protected SmartCardID getSmartCardIDImpl() {
      ...
      CommandAPDU readDKISBinaryAPDU = new CommandAPDU((byte)0, (byte)-80, (byte)0, (byte)0, 8);
      CommandAPDU selectUISFileAPDU = getSelectFileCommand(new byte[]{0, 10});
      CommandAPDU readUISBinaryAPDU = new CommandAPDU((byte)0, (byte)-80, (byte)0, (byte)0, 32);
      CommandAPDUGroup commandAPDUGroup = new CommandAPDUGroup(new CommandAPDU[]{selectDKISFileAPDU, readDKISBinaryAPDU, selectUISFileAPDU, readUISBinaryAPDU});
      ...
      return new SmartCardID(idLong, cardLabel, this.getSmartCard());
   }
