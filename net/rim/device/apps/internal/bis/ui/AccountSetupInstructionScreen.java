...
public final class AccountSetupInstructionScreen extends UserSettingsScreen {
   ...
   public final void refresh(Hashtable screenParams) {
      ...
      String config = MessageFormat.format(ApplicationResources.getString(236), new String[]{session.getIntegrationEmail()});
      this.addContentField(new LabelField(config));
      this.addContentLineBreak();
      if (!session.isAutoAuth()) {
         LabelField follow = new LabelField(ApplicationResources.getString(238));
      ...
      LabelField toAdd = new LabelField(MessageFormat.format(ApplicationResources.getString(239), new String[]{session.getIntegrationEmail()}));
      ...
      if (session.isAutoAuth()) {
         ...
         this.addContentField(orLabel);
         this.addContentLineBreak();
         String toConfig = MessageFormat.format(ApplicationResources.getString(240), new String[]{session.getIntegrationEmail()});
         this.addContentField(new TextField(null, toConfig, toConfig.length(), 9007199254740992L));
         this.addContentLineBreak();
         createLink = new LinkField(ApplicationResources.getString(241));
         this.addContentField(createLink);
         String emailSent = MessageFormat.format(ApplicationResources.getString(243), new String[]{session.getIntegrationEmail()});
         this.addContentField(new TextField(null, emailSent, emailSent.length(), 9007199254740992L));
      }
