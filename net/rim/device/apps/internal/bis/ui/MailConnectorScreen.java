...
public final class MailConnectorScreen extends UserSettingsScreen {
   ...
   public final void refresh(Hashtable screenParams) {
      ...
      if (ClientSessionState.getInstance().getUserInfo().isAutoAuth()) {
         ...
         String config = MessageFormat.format(ApplicationResources.getString(267), new String[]{session.getIntegrationEmail()});
         this.addContentField(new TextField(null, config, config.length(), 9007199254740992L));
         ...
         String emailSent = MessageFormat.format(ApplicationResources.getString(268), new String[]{session.getIntegrationEmail()});
         ...
         this.addContentField(new LabelField(mailConnectorLabelText));
         Button closeButton = new Button(ApplicationResources.getString(15));
