...
final class TaskSearchable implements Searchable {
   @Override
   ...
   public final String getName(long id) {
      return ApplicationEntryPoint.removeHotkeyFromDescription(TaskResources.getString(0));
   }
   ...
   public final EncodedImage getIcon(long id) {
      EncodedImage tempImage = ThemeManager.getActiveTheme().getImage("net_rim_bb_task_app", true);
   ...
   @Override
   public final SearchResultCollection search(long[] ids, Object searchCriteria) {
      SearchResultCollection results = new TaskSearchResultCollection(searchCriteria);
      results.loadFrom(TaskCollectionImpl.getInstance());
      return results;
   }
}
