...
public final class InputMaskTextFilter extends TextFilter {
   ...
   private final boolean validateChar(char c, char format) {
      ...
      if (CharacterUtilities.isSymbol(c) || CharacterUtilities.isPunctuation(c) || c == ' ') {
         check = true;
      }

      switch (format) {
         case 'A':
            if (!CharacterUtilities.isUpperCase(c) && !check) {
               return false;
            }
         ...
         case 'M':
         case 'm':
            if (!CharacterUtilities.isLetter(c) && !Character.isDigit(c) && c != '\n' && !check) {
               return false;
            }

            return true;
         case 'X':
            if (!CharacterUtilities.isUpperCase(c) && !Character.isDigit(c) && !check) {
               return false;
            }

            return true;
         case 'a':
            if (!CharacterUtilities.isLowerCase(c) && !check) {
               return false;
            }
         ...
         case 'n':
            ...
            return true;
         case 'x':
            if (!CharacterUtilities.isLowerCase(c) && !Character.isDigit(c) && !check) {
               return false;
            }
   ...
   private final char autoCase(char key, char format) {
      ...
      if ((format == 'A' || format == 'X') && key >= 'a' && key <= 'z') {
         ...
         return (format != 'N' && format != 'n' || (key < 'a' || key > 'z') && (key < 'A' || key > 'Z') && key != 137)
               && (format != 'N' || key != ' ' || Keypad.getHardwareLayout() != 1364346180)
            ? key
   ...
   public final char convert(char character, int status) {
      ...
      return this.isLiteralAtIndex(position) ? character : this.autoCase(character, this.getFormatAtIndex(position));
   }
   ...
   public final boolean validate(AbstractString text) {
      if (text != null && !this._validationDisabled) {
   ...
   public final boolean validate(char character, int index) {
      if (this._validationDisabled) {
         return true;
      }

      if (index == -1) {
         index = this._edit.getCursorPosition();
      }

      return this.validate(character, index, true);
   }

   @Override
   public final boolean validate(char character, AbstractString text, int position) {
      return this.validate(character, position);
   }
   ...
   public final boolean validate(String value) {
      ...
      for (int i = 0; i < count; i++) {
         if (!this.validateChar(value.charAt(i), i)) {
            return false;
         }
      }
   ...
   public final void setValidation(boolean validationEnabled) {
      this._validationDisabled = !validationEnabled;
   }
}
