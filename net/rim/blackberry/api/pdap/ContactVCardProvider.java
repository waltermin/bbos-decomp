...
class ContactVCardProvider implements VCardProvider {
   ...
   private String[] _nameArray = new String[5];
   private String[][] _addressArray = new String[2][7];
   ...
   public ContactVCardProvider(ContactImpl contact) {
      ...
      try {
         for (int i = 0; i < 2; i++) {
            this._addressArray[i] = this._contact.getStringArray(100, i);
         }
      } finally {
         return;
      }
   ...
   private void setNameField(int arrayindex, String value) {
      if (this._contact.countValues(106) > 0) {
   ...
   public boolean hasNextAddress() {
      return ++this._nextAddress < this._contact.countValues(100);
   }
   ...
   public int getAddressType(int firstOrNext) {
      int index = 0;
      if (firstOrNext == 2) {
         index = this._nextAddress;
      }

      return (this._contact.getAttributes(100, index) & 512) != 0 ? 46 : 30;
   }

   private String getAddressField(int firstOrNext, int field) {
      return firstOrNext == 1 ? this._addressArray[0][field] : this._addressArray[this._nextAddress][field];
   }
   ...
   public String getExtendedAddress(int firstOrNext) {
      return this.getAddressField(firstOrNext, 1);
   }

   @Override
   public String getStreet(int firstOrNext) {
      return this.getAddressField(firstOrNext, 2);
   }

   @Override
   public String getLocality(int firstOrNext) {
      return this.getAddressField(firstOrNext, 3);
   }

   @Override
   public String getRegion(int firstOrNext) {
      return this.getAddressField(firstOrNext, 4);
   }

   @Override
   public String getPostalCode(int firstOrNext) {
      return this.getAddressField(firstOrNext, 5);
   }

   @Override
   public String getCountry(int firstOrNext) {
      return this.getAddressField(firstOrNext, 6);
   }
   ...
   public void addTelephone(int type, String number) {
      try {
         if ((type & 2) != 0) {
            ...
            if ((type & 16) == 0) {
               this._contact.addString(115, 32, number);
               return;
            }
   ...
   public void setUrl(String url) {
      try {
         this._contact.addString(118, 0, url);
      } finally {
         return;
      }
   }
   ...
   public String getUrl() {
      return this._contact.countValues(118) > 0 ? this._contact.getString(118, 0) : null;
   }
