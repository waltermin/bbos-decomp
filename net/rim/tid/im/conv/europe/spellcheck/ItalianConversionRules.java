...
public class ItalianConversionRules implements LocaleConversionRules {
   private ItalianConversionRules$Conversion _currentConversion;
   ...
   public boolean exists(Word word) {
      ...
      label40: {
         try {
            ...
            ItalianConversionRules$Conversion conversion = this.checkForContractionConversion(lower.text, lower.start, lower.length());
            if (conversion != null) {
   ...
   public int wordExists(char[] word, int offset, int len, char[] lcWord) {
      ItalianConversionRules$Conversion conversion = this.checkForContractionConversion(lcWord, 0, len);
      if (conversion != null) {
   ...
   private ItalianConversionRules$Conversion checkForContractionConversion(char[] word, int offset, int len) {
      if (len < 3) {
         return null;
      }
      ...
      for (int i = offset + 1; i < offset + len; i++) {
         if (word[i] == '\'') {
            ItalianConversionRules$ContractionConversion conv = CONTRACTION;
            conv.setContractionPrefix(word, offset, i - offset);
            return conv;
         }
      }
